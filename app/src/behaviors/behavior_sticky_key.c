...
static struct active_sticky_key *store_sticky_key(uint8_t source, uint32_t position, uint32_t param1,
                                                  uint32_t param2,
                                                  const struct behavior_sticky_key_config *config) {
...
static int on_sticky_key_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    sticky_key = store_sticky_key(event.source, event.position, binding->param1, binding->param2, cfg);
    if (sticky_key == NULL) {
